...
<%--    搜索表单--%>
    ...
    <script inline="javascript">
        ...
        layui.use(['rate','table','form'], function(){
            ...
            table.render({
                elem: '#table-question'
                ,url:  'http://localhost:8080<%=request.getContextPath()%>/multiQuestion/getQuestionList' //数据接口
                ,page: true //开启分页
                ...
                ,cols: [[ //表头
                    ...
                    ,{field: 'content', title: '详细', width: 80,align: 'center'}
                    ...
                    ,{field: 'section', title: '所属章节',width: 80, align: 'center'}
            ...
            table.on('toolbar(table-question-filter)', function(obj){
                if(obj.event == 'add'){
                    var index = layer.open({
                        ...
                        shadeClose: true,
                        area: ['80%', '80%'],
                        content: "<%=request.getContextPath()%>/multiQuestion/add",
                    });
                    ...
                    layer.confirm('您确认要删除选中的记录吗？', {icon: 3, title:'是否删除'}, function(index){
                        var url = "<%=request.getContextPath()%>/multiQuestion/deleteByIds";
                        var params = {
                            _method:"DELETE",
            ...
            table.on('tool(table-question-filter)', function(obj){ //注：tool 是工具条事件名，test 是 table 原始容器的属性 lay-filter="对应的值"
                ...
                if(layEvent === 'del'){ //删除
                    layer.confirm('真的删除么', function(index){
                        ...
                        idStr = id + "";
                        var url = "<%=request.getContextPath()%>/multiQuestion/deleteById?questionId=" + idStr ;
                        var params = {
                            _method:"DELETE"
                        }
                        $.post(url,params,function (response){
                    ...
                    layer.open({
                        ...
                        shadeClose: false,
                        area: ['80%', '80%'],
                        content: "<%=request.getContextPath()%>/multiQuestion/questionDetail?questionId=" + data.questionId
                    });
                }
            });
        });
